// ===============================================================================
// APEX TITAN OMNISCIENT MASTER v27.0 - HIGH-FREQUENCY CLUSTER
// ===============================================================================
...
// --- CONFIGURATION (Merged with v27.0 Snippets) ---
const GLOBAL_CONFIG = {
    TARGET_CONTRACT: process.env.TARGET_CONTRACT || "0x83EF5c401fAa5B9674BAfAcFb089b30bAc67C9A0", 
    BENEFICIARY: "0x4B8251e7c80F910305bb81547e301DcB8A596918", // Beneficiary Updated
    ...
    // STRATEGY SETTINGS
    WHALE_THRESHOLD: parseEther("10.0"), // Trigger for Omniscient Snipes
    MIN_LOG_ETH: parseEther("10.0"),     // Leviathan Log Confirmations
    MIN_WHALE_VALUE: 0.1,                // Visual Heartbeat Trigger
    GAS_LIMIT: 1400000n,                 // Safety buffer for complex L2 routes
    PORT: process.env.PORT || 8080,
    MARGIN_ETH: "0.015",                 // Profit floor
    PRIORITY_BRIBE: 15n,                 // 15% Tip for block priority

    // 🌍 NETWORKS (Using keys from your real-time scanner snippet)
    NETWORKS: [
        {
            name: "ETH_MAINNET",
            chainId: 1,
...
if (cluster.isPrimary) {
    ...
    console.log(`${TXT.bold}${TXT.gold}║   ⚡ APEX TITAN MASTER v27.0 | OMNISCIENT CLUSTER     ║${TXT.reset}`);
    console.log(`${TXT.bold}${TXT.gold}║   DUAL SNIPE: INFURA MAINNET + ALCHEMY BASE           ║${TXT.reset}`);
    console.log(`${TXT.bold}${TXT.gold}╚════════════════════════════════════════════════════════╝${TXT.reset}\n`);
...
async function initWorker(CHAIN) {
    ...
    try {
        const server = http.createServer((req, res) => {
            if (req.url === '/status') {
                ...
                res.end(JSON.stringify({ status: "ONLINE", chain: CHAIN.name, mode: "OMNISCIENT_v27" }));
    ...
    if (cleanKey.length !== 66 && cleanKey.length !== 64) throw new Error("Invalid Private Key.");
    ...
    wsProvider.on("pending", async (txHash) => {
        try {
            ...
            if (scanCount % 25 === 0 && (cluster.worker.id % 6 === 0)) {
               process.stdout.write(`\r${TAG} ${TXT.blue}⚡ SCANNING MEMPOOL${TXT.reset} | Txs: ${scanCount} | ETH: $${currentEthPrice.toFixed(2)} `);
            }
            ...
            if (valueWei >= GLOBAL_CONFIG.WHALE_THRESHOLD) {
                console.log(`\n${TAG} ${TXT.magenta}🚨 WHALE SPOTTED: ${formatEther(valueWei)} ETH | Hash: ${txHash.substring(0, 10)}...${TXT.reset}`);
                await attemptStrike(provider, wallet, titanIface, gasOracle, poolContract, currentEthPrice, CHAIN, flashbotsProvider, "OMNISCIENT");
            }
            
            // Vector 2: Triangle Volatility Probe
...
async function executeStrikeInternal(provider, wallet, strikeData, loanAmount, gasOracle, ethPrice, CHAIN, flashbotsProvider, mode) {
    try {
        ...
        if (rawProfit > totalCostThreshold) {
            ...
            console.log(`\n${TXT.green}${TXT.bold}💎 ${mode} STRIKE AUTHORIZED${TXT.reset} | Profit: ${formatEther(cleanProfitEth)} ETH`);
