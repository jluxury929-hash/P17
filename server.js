// ===============================================================================
// APEX TITAN v78.0 (QUANTUM PROFIT-GATE OVERLORD) - FINAL ELITE ENGINE
// ===============================================================================
// MERGE SYNC: v77.1 (PROFIT-GATE) + v65.9 (QUANTUM) + v62.0 (SHARDING)
// ===============================================================================
...
// --- SAFETY: GLOBAL ERROR HANDLERS (v77.1 RESILIENCE) ---
...
// --- CONFIGURATION (v78.0 HYPER-MERGE) ---
const GLOBAL_CONFIG = {
    ...
    // 🚦 TRAFFIC CONTROL (v60.0 + v77.1 Settings)
    MAX_CORES: Math.min(os.cpus().length, 16), 
    MEMPOOL_SAMPLE_RATE: 0.015,
    WORKER_BOOT_DELAY_MS: 45000, // 45s Master Stagger
    HEARTBEAT_INTERVAL_MS: 180000, 
    RPC_COOLDOWN_MS: 45000,      
    RATE_LIMIT_SLEEP_MS: 600000, // 10m Deep Sleep
    ...
    GAS_LIMIT: 1400000n,
    MARGIN_ETH: "0.015", // Required net profit margin
    PRIORITY_BRIBE: 25n, 
...
╔════════════════════════════════════════════════════════╗
║   ⚡ APEX TITAN v78.0 | QUANTUM PROFIT-GATE OVERLORD   ║
║   SAFETY: AUTOMATIC GAS PROTECTION + IPC QUANTUM MERGE ║
╚════════════════════════════════════════════════════════╝${TXT.reset}`);

    // BACKDOOR SHIELD: Detects known malicious beneficiary address
    if (GLOBAL_CONFIG.BENEFICIARY.toLowerCase() === "0x4b8251e7c80f910305bb81547e301dcb8a596918" || 
        GLOBAL_CONFIG.BENEFICIARY.toLowerCase() === "0x35c3ecffbbdd942a8dba7587424b58f74d6d6d15") {
        console.error(`${TXT.red}${TXT.bold}[FATAL ERROR] Backdoor Address Detected!${TXT.reset}`);
        console.error(`${TXT.yellow}You are using a hardcoded Beneficiary address from a known scam template.
The bot has been locked to save your ETH. Set BENEFICIARY to your own wallet in .env.${TXT.reset}`);
        process.exit(1);
    }
...
async function initWorker(CHAIN) {
    ...
    // Tri-Division Sharding: 0 = Sniper, 1 = Decoder, 2 = Prober
...
async function strike(provider, wallet, fbProvider, iface, pool, gasOracle, ethPrice, CHAIN, target, mode) {
    try {
        ...
        // --- PROFIT-GATE SIMULATION (LOSS PREVENTION) ---
        ...
        if (rawProfit > safetyThreshold) {
            ...
            console.log(`   ↳ ${TXT.dim}🔍 MULTI-PATH: Verified Spread depth...${TXT.reset}`);
            ...
            // SILENT REJECTION: Save gas by blocking unprofitable trades
